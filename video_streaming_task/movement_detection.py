...
def movement_detection(file, max=15):
    ...
    while True:
        ...
        for c in contours:
            # 面积/阈值
            if cv.contourArea(c) < 1700:  # 对于矩形区域，只显示大于给定阈值的轮廓，所以一些微小的变化不会显示。对于光照不变和噪声低的摄像头可不设定轮廓最小尺寸的阈值
                continue
            count_s = count_s + 1
...
for file in list_f:
    print(file)
    movement_detection("video\\" + file, max=7)
    movement_detection2("video\\" + file, max=7)
    break
